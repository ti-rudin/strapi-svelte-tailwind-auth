...
<script lang="ts" context="module">
	...
	export const load: Load = async ({ fetch }) => {
		...
		if (edit) {
			const res = await fetch('http://ds.rudin.ru:1337/api/posts/' + edit);
...
<script lang="ts">
	...
	let author;
	onMount(() => {
		if (!$user) { goto('/login');} else {author = $user.id;}
	});
	...
	async function createPost() {
		...
		const res = await fetch('http://ds.rudin.ru:1337/api/posts', {
			method: 'POST',
			headers: {
			...
			body: JSON.stringify({ data: { title, description, content, author } })
		});
		if (!res.ok) {
